...
packages = c("downloader", "haven", "dplyr", "rdhs", "sf", "readr", "ggplot2", 
             "here", "s2")
...
These error features were found by pulling in the MWI_adm shapefile (TAs) and MWGE62FL shapefile (DHS Household Survey Cluster points), then using the count points in polygon tool to find which traditional authorities did and did not contain household points.
The workflow for this troubleshooting process can be found [here](https://github.com/daptx/RP-Malcomb/issues/6).
Ultimately, this source of error was the reason behind this first notebook, to fix the traditional authority geometries before attempting to replicate the original results of Malcomb et al (2014).

With the new traditional authorities vectors (234 features) in QGIS, a georeferenced shapefile of the figure 4 from Malcomb et al (2014), the adaptive capacity map, was converted into a .csv, then joined to the traditional authorities derived from this code by ID_2 (the column feature used to distinguish TAs).
The custom prefix `gref_` was used. Once created, a field calculator was used to create a 'capacity' column, turning the `gref_resilience` scores into integers and fixing the naming syntax for the remainder of analysis.
In Malcomb et al (2014), capacity and resilience were terms used loosely and interchangeably, making it essential to follow one naming schema in this analysis: `adaptive capacity` or `adaptive capacity scores`.
After, this final polygon layer of 234 features and 23 variables (geometry included) was exported as `ta_fig4_v.gpkg` and is brought in at the start of the `02-Analysis.Rmb`  notebook.
...
# the following prompts could be changed to popups
