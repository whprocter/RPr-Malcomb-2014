...
Joseph Holler, Kufre Udoh, Drew An-Pham, and 
...
packages = c("downloader", "haven", "stars", "dplyr", "sf", "rdhs", "classInt", 
             "readr", "ggplot2", "here", "s2", "pastecs", "cartography", "sp")
...
# Load lakes and dissolve into a single multi-part feature
lakes <- read_csv(here(public_r, "major_lakes.csv")) %>% 
  st_as_sf(wkt = "the_geom", crs = 4326) %>%
  st_union(by_feature = FALSE) %>%  # dissolve
  st_sf %>%
...
# we want flood to have 20% weight so we multiply by 5, 
# as is the data has a range of 0-4
flood <- fl
flood[is.na(ta_capacity_r)] = NA
flood = flood * 5
...
final <- (40 - ta_capacity_r) + lhz_sensitivity_r + drought + flood 
...
map_2010legend <- map_2010legend %>% append(c(
    "Missing Data" = "#FFC389",
...
# create the map
map_2010 = ggplot() +
  geom_sf(data = ea, aes(fill = EA), color = NA) +
  geom_sf(
    data = ta_capacity_v,
  ...
  scale_x_continuous(breaks = c(33, 34, 35, 36)) +
  labs(
    fill = "Adaptive Capacity Score"
  ) #+
  #theme_minimal() +
 # theme(legend.title = element_blank())
# is the minimal theme required?
...
```{r compare vulnerability, warning=FALSE, fig.width=4, fig.height=4}
orfig5vect = 
...
abline(lm(fig5comppts$rp ~ fig5comppts$or))
...
ac_diff_map = ggplot() +
  ...
  scale_x_continuous(breaks = c(33,34,35,36)) +
  labs(
    title = "Fig. 4 Replication Comparison",
    subtitle = "Reproduction - Original",
    fill = "Reproduction - \nOriginal"   ) +
  theme_minimal() +
  theme(legend.title = element_blank())
...
vuln_diff_map = ggplot() +
  ...
  scale_x_continuous(breaks = c(33,34,35,36)) +
  labs(
    title = "Fig. 5 Replication Comparison",
    fill = "Reproduction -\nOriginal",
  ) +
  theme_minimal() +
  theme(
