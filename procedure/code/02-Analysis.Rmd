...
if (!file.exists(here("procedure","environment","r_environment.txt"))) {
  writeLines(capture.output(sessionInfo()),here("procedure","environment","r_environment.txt"))
} else {
  writeLines(capture.output(sessionInfo()),here("procedure","environment",paste0("r-environment-",Sys.Date(),".txt",sep="")))
}
```
...
cluster_ta <- dhsclusters_2010 %>% 
  st_join(ta_gref) %>% 
  rename(ta_id = ID_2, urban_rural = URBAN_RURA) %>%
  st_drop_geometry() %>% 
  select(DHSCLUST, ta_id, urban_rural)
...
# create list of households with missing data for adaptive capacity indicators
rmv_2010 <- dhshh_2010 %>%  
  filter(
    HV246A == 98 |
    HV246A == 99 |
...
paste(nrow(rmv_2010), "household surveys with missing attribute data")
```
