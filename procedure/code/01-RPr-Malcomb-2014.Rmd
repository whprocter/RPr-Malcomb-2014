...
# Loads the pre-aggregated (publicly-avaialable adaptive capacity data)
...
mean_vulnerability <- aggregate(vulnerability_r, ta_v, FUN = mean) %>%
  st_as_sf()
...
continuous_layer <- tm_shape(mean_vulnerability) +
  tm_fill(col = "vulnerability", palette = c("#FFFF75", "#CF4611")) +
...
discrete_layer <- tm_shape(ta_cons_v) +
  tm_fill(col = "TYPE_2", palette = hcl.colors(4, "greens"), alpha = 0.7, title = "") +
  tm_borders(col = "grey98")
# Create additional map layers for lakes_v and ta_v 
...
# Extract TA names and vulnerability values
ta_vulnerability_table <- data.frame(
  TA = mean_vulnerability$TA_Name,
  Vulnerability = mean_vulnerability$vulnerability
)
...
# Assuming you have 'mean_vulnerability' with TA geometries and vulnerability values
# and 'ta_v' with the 'name_2' column

# Add the 'name_2' column to 'mean_vulnerability' by matching geometries
mean_vulnerability_with_names <- st_join(mean_vulnerability, ta_v[, "NAME_2"], join = st_intersects)

# Create a dataframe with two columns
ta_vulnerability_dataframe <- data.frame(
  TA = mean_vulnerability_with_names$NAME_2,  # Use the 'name_2' column as the TA name
  Mean_Vulnerability = mean_vulnerability_with_names$vulnerability
)

# Sort the dataframe in descending order of vulnerability
ta_vulnerability_dataframe <- ta_vulnerability_dataframe[order(-ta_vulnerability_dataframe$Mean_Vulnerability), ]

# Display the sorted dataframe
print(ta_vulnerability_dataframe)
...
# Create a histogram of the 'diffv' variable
...
# Create a histogram of the 'diffv' variable
vuln_diff_histogram <- ggplot(as.data.frame(vulnerability_r["diffv"]), aes(x = diffv)) +
  geom_histogram(
    bins = 30,  # You can adjust the number of bins as needed
    fill = "#3182bd",  # Adjust fill color
    color = "#6baed6"  # Adjust border color
...
This reproduction study was chocked full of frictions to reproduction.
...
The second was to amend the Figure 5 output (vulnerability map) to show vulnerability by TA, rather than just a raster grid.  Showing vulnerability by TA allows us to easily include natural parks/wildlife refuges, as well as identify areas where there was missing data for the TAs (stemming from HH surveys).
