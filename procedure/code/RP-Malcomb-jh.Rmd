...
lhz = read_sf(here("data", "raw", "public", "MW_LHZ_2009.shp")) %>% st_make_valid()
...
hh_capacity_2010 = dhshh_2010 %>%
  ...
  mutate(
    ...
    sexcat = percent_rank(desc(HV219)) * 4,
...
ta_hh_capacity_2010 <- hh_capacity_2010 %>%
...
hh_stats <- stat.desc(ta_hh_capacity_2010[,21:33]) %>%
  mutate_if(is.numeric, round, digits=3)
```

```{r extract traditional authorities without data}
hh_null <- ta_hh_capacity_2010[is.na(ta_hh_capacity_2010$capacity),] 

st_write(hh_null, here("data", "derived", "public", "hh_null.shp"), append=FALSE)
# run the function up to na.rm = F, then run this function
#4032 features
```

```{r remove nulls}
ta_hh_capacity_2010 <- ta_hh_capacity_2010 %>%
filter(!is.na(capacity)) %>% # removing incomplete surveys 
ungroup
#19996 features (from the initial 24028 --> removed 4032 NA features)

hh_stats_2 <- stat.desc(ta_hh_capacity_2010[,21:33]) %>%
  mutate_if(is.numeric, round, digits=3)
```

```{r capacity for traditional authorities 2010; aggregates from household level}
  ta_capacity_2010 <- ta_hh_capacity_2010 %>% 
  group_by(ta_id) %>%
  summarize(
...
ta = mutate(ta, capacity_2010 = capacity_2010 * 25) # --> THIS LINE IS NOW REMOVED
# 256 features 
...
lhz_filtered <- lhz_scores[-(17:18),] #removes lhz zones 18 & 19 (urban area & national parks)

lhz_good  = lhz %>%
  left_join(lhz_filtered, by = ("LZCODE" = "LZCODE")) %>%
...
# making capacity rasters 
lhz_capacity_new = st_rasterize(lhz_good[,'livelihoodSensitivity_new'], blank)
```

```{r a table for livelihood sensitivity data reporting all of the individual variables used, & the calculated variables}

trial <- lhz_good %>% 
  select(LZNAMEEN, LZCODE, sof_crop, sof_livestock, soc_labour, soc_total, cp_tobacco, cp_sugar, cp_tea,	cp_coffee, se_firewoord,	se_grass,	se_wildfood,	se_charcoal,	se_matmaking,	se_basket) 
trial2 <- st_drop_geometry(trial) %>%
  na.omit
new <- list("Source of Food: Crops" =
         list("min" = ~min(trial2$sof_crop),
              "max" = ~max(trial2$sof_crop),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$sof_crop, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$sof_crop)),
        "Source of Food: Livestock" =
          list("min" = ~min(trial2$sof_livestock),
              "max" = ~max(trial2$sof_livestock),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$sof_livestock, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$sof_livestock)),
        "Source of Cash: Labour" =
          list("min" = ~min(trial2$soc_labour),
              "max" = ~max(trial2$soc_labour),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$soc_labour, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$soc_labour)),
        "Source of Cash: Total" =
          list("min" = ~min(trial2$soc_total),
              "max" = ~max(trial2$soc_total),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$soc_total, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$soc_total)),
        "Crop Production: Tobacco" =
          list("min" = ~min(trial2$cp_tobacco),
              "max" = ~max(trial2$cp_tobacco),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$cp_tobacco, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$cp_tobacco)),
        "Crop Production: Sugar" =
          list("min" = ~min(trial2$cp_sugar),
              "max" = ~max(trial2$cp_sugar),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$cp_sugar, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$cp_sugar)),
        "Crop Production: Tea" =
          list("min" = ~min(trial2$cp_tea),
              "max" = ~max(trial2$cp_tea),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$cp_tea, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$cp_tea)),
        "Crop Production: Coffee" =
          list("min" = ~min(trial2$cp_coffee),
              "max" = ~max(trial2$cp_coffee),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$cp_coffee, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$cp_coffee)),
        "Self-Employment: Firewood" =
          list("min" = ~min(trial2$se_firewoord),
              "max" = ~max(trial2$se_firewoord),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$se_firewoord, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$se_firewoord)),
        "Self-Employment: Grass" =
          list("min" = ~min(trial2$se_grass),
              "max" = ~max(trial2$se_grass),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$se_grass, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$se_grass)),
        "Self-Employment: Wildfood" =
          list("min" = ~min(trial2$se_wildfood),
              "max" = ~max(trial2$se_wildfood),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$se_wildfood, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$se_wildfood)),
        "Self-Employment: Charcoal" =
          list("min" = ~min(trial2$se_charcoal),
              "max" = ~max(trial2$se_charcoal),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$se_charcoal, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$se_charcoal)),
        "Self-Employment: Mat Making" =
          list("min" = ~min(trial2$se_matmaking),
              "max" = ~max(trial2$se_matmaking),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$se_matmaking, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$se_matmaking)),
        "Self-Employment: Basket" =
          list("min" = ~min(trial2$se_basket),
              "max" = ~max(trial2$se_basket),
              "mean (sd)" = ~qwraps2::mean_sd(trial2$se_basket, denote_sd = "paren"),
              "count" = ~qwraps2::n_perc0(trial2$se_basket))
       )
whole <- summary_table(trial2, new)
whole
```

```{r summary table with lhz data}
lhz_stats <- stat.desc(lhz_good[,14:36]) %>%
  mutate_if(is.numeric, round, digits=3)
lhz_stats
```
...
```{r function to calculate vulnerability}
  ...
  final = 100 - (ta_capacity + lhz_capacity_new + (40 - (drought + flood))) # 100 = highest vulnerability, 0 = lowest vulnerability
```
...
#rasterize 
ta_new = ta_2010[!is.na(ta_2010$vuln), 18]
ta_new = st_rasterize(ta_new[1])
```
...
map_2010 = ggplot() +
  ...
  geom_sf(
    ...
    #  unique(ta_2010$capacity_2010_brks) allows you to see breaks
    values = list(
      "Missing Data" = "#FFC389",
...
vuln_max <- max(ta_new[[1]],na.rm = TRUE)
vuln_min <- min(ta_new[[1]],na.rm = TRUE)
``` 
...
write_stars(ta_new, here("data","derived","public","ta_capacity.tif"))
...
orfig5rast = st_rasterize(orfig5vect["bg_mean"], template=ta_new)
...
ta_new = ta_new %>% 
  mutate(rp =
           (vuln - min(ta_new[[1]], na.rm= TRUE)) /
           (max(ta_new[[1]], na.rm= TRUE) -
            min(ta_new[[1]], na.rm= TRUE)
        )
)  # rp is Re-scaled from 0 to 1 with (value - min)/(max - min)

fig5comp = c( select(ta_new,"rp"), select(orfig5rast,"or"))
