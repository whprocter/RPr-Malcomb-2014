...
lhz_sensitivity = lhz %>%
  ...
  mutate(
    ...
    cashCropIncome = percent_rank(desc(pctIncCashCrops)) * 4, #high values mean lower senstivity (b/c of desc)
    newDisaster = percent_rank(desc(disasterCoping)) * 4, #high values mean lower senstivity (b/c of desc)
  ) %>%
  rowwise %>%
  mutate(
...
# livelihood sensitivity range iis now .235 to .490
```
...
```{r function to calculate vulnerability}
  ...
  final = ta_capacity + lhz_capacity + (40 - (drought + flood)) 
```

```{r creating final vulnerability layers}
# ta_final = vulnerability(ta_capacity)

ta_2010$vuln = aggregate(final,ta_2010,mean)$capacity_2010
```
...
orfig5rast = st_rasterize(orfig5vect["bg_mean"], template=ta_final)
