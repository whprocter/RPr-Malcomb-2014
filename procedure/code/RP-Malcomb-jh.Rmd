...
# st_write(, here("data", "derived", "public", "hh_null.csv"), append=FALSE))
```
...
ta_corrected = st_difference(st_transform(filter(ta_errors), 3395), st_buffer(st_transform(lakes, 3395), 625)) %>%
  st_transform(4326) %>%
  st_cast ("POLYGON", warn=FALSE)

# select out 5 largest polygon features (the ones we actually care about)
ta_corrected <- ta_corrected[c(1:4,17),]
ta_corrected <- ta_corrected %>%
  mutate(ID_2 = ID_2 + row_number() * 1000) %>% #gives unique IDs different values
  select(-c(EA)) %>%
  st_cast ("MULTIPOLYGON", warn=FALSE)

ta_corrected <- bind_rows(ta, ta_corrected)

st_write(ta_corrected, here("data", "scratch", "ta_corrected.shp"), append=FALSE) 
```
...
dhsclusters_2010 = readRDS(here("data", "raw", "private", "datasets", "MWGE62FL.rds")) %>%
  ...
  st_join(st_transform(select(ta, ID_2),3395)) %>% #FOR revision: you want to join to good TAs, refer to workflow 7/6 & count dhs clusters + households #survey clusters to TAs #keep such id unique
...
ta_null_cap <- ta[is.na(ta$capacity_2010),]
#34 features
