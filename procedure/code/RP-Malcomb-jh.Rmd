...
### Authors: Kufre Udoh, Joseph Holler, Drew An-Pham, and Middlebury College Spring 2019 + 2021 Geography 323 Class
...
```{crerates RDS file for DHS downloaded}
# saveRDS(dhs_downloads,"data/raw/private/datasets/dhs_downloads.rds")
...
ta_capacity_2010b <- hh_capacity_2010 %>%
...
lhz_sensitivity = lhz %>%
  ...
  mutate(
    ownCrops = percent_rank(pctOwnCrops) * 4, # high values means lower sensitivity 
...
```{pull in ALL data at once}
# LHZ Data
lhz_scores_new = read.csv(here(private_r, "lhz_test2.csv"))

lhz_good  = lhz %>%
  left_join(lhz_scores_new, by = ("LZCODE" = "LZCODE")) %>%
...
```{r function to calculate vulnerability}
  ...
  final = 100 - (ta_capacity + lhz_capacity + (40 - (drought + flood))) # 100 = highest vulnerability, 0 = lowest vulnerability
```
...
```{find break & limit values for vulnerability map}
max(ta_new[[1]],na.rm = TRUE)
min(ta_new[[1]],na.rm = TRUE)
# copy & paste above code into console, block won't run otherwise (reason?)
``` 
...
vuln_map = ggplot() +
  ...
  scale_fill_gradient(
    low = "#FFFF75",
    high = "#CF4611",
    breaks = c(0.9499073, 61.00681),
    labels = c("Lower Vulnerability", "Higher Vulnerability"),
    na.value = "transparent",
    guide = "colourbar",
    limits = c(0.9499073, 61.00681)
  ) +
  scale_x_continuous(breaks = c(33,34,35,36)) +
...
ggsave(
  here("results","assets","fig4rep_04BG.png"),
  plot = map_2010,
  width = 8.5,
  height = 11,
  units = "in"
...
ggsave(
  here("results","assets","fig5rep_04BG.png"),
  plot = vuln_map,
  width = 8.5,
  height = 11,
  units = "in"
...
vuln_diff_map = ggplot() +
  ...
  scale_fill_gradient2(
    low = "#e9a3c9",
    high = "#beaed4",
    breaks = c(-0.974405,0.974405),
    labels = c("Negative Difference", "Positive Difference"),
    na.value = "transparent",
    guide = "colourbar",
    limits = c(-0.974405,0.974405) #you want centered around zero, so make larger # the ends
  ) +
  scale_x_continuous(breaks = c(33,34,35,36)) +
...
ggsave(
  here("results","assets","fig4comp_04BG.png"),
  plot = ac_diff_map,
  width = 8.5,
  height = 11,
  units = "in"
...
ggsave(
  here("results","assets","fig5comp_04BG.png"),
  plot = vuln_diff_map,
  width = 8.5,
  height = 11,
  units = "in"
...
```{exporting comparison as geotiff}
write_stars(fig5comp, "fig5diff.tif", layer = "diff")
write_stars(fig5comp, "fig5or.tif", layer = "or")
write_stars(fig5comp, "fig5rp.tif", layer = "rp")
# copy & paste above code into console, block won't run otherwise (reason?)
```